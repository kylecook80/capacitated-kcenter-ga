(function() {
  ...
  $('#get-filename').on('change', function() {
    ...
    return generate_graph(data.data, modulus, null);
  });

  $('#input-center').keypress(function(event) {
    if (event.which === 13) {
      return $('#add-center').click();
    }
  });

  $('#gen-file').on('click', function() {
    var array, centers, name, node_list;
    name = $('#gen-filename').val();
    centers = [];
    $('#centers-list').children().each(function(idx, el) {
      return centers.push($(el).html().split('</i> ')[1].split(','));
    });
    $('#container').empty();
    node_list = (function() {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        array = _ref[_i];
        _results.push(array.map(function(item) {
    ...
    return $('#get-filename').val(name + '.json');
  });
